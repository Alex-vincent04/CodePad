from fastapi import APIRouter, File, Form, Request, UploadFile, status, HTTPException, WebSocket, WebSocketDisconnect, BackgroundTasks
...
async def _push_task_to_github(task: dict, repo: str, token: str) -> tuple[str | None, str | None, str | None]:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        # Multi-part frontend push if applicable
        if task.get("type") == "frontend" and any([task.get("html"), task.get("css"), task.get("js")]):
            if task.get("html"):
                await _upsert_github_file(client, owner, repo_name, branch, f"{dir_path}/index.html", 
                                        base64.b64encode(task["html"].encode("utf-8")).decode("ascii"), f"Sync index.html: {task_title}", token)
            if task.get("css"):
                await _upsert_github_file(client, owner, repo_name, branch, f"{dir_path}/style.css", 
                                        base64.b64encode(task["css"].encode("utf-8")).decode("ascii"), f"Sync style.css: {task_title}", token)
            if task.get("js"):
                await _upsert_github_file(client, owner, repo_name, branch, f"{dir_path}/script.js", 
                                        base64.b64encode(task["js"].encode("utf-8")).decode("ascii"), f"Sync script.js: {task_title}", token)
        else:
...
async def _push_project_to_github(
    ...
    code_tasks = [
        ...
        if (task.get("code") or "").strip() 
        or (task.get("type") == "frontend" and any([(task.get("html") or "").strip(), (task.get("css") or "").strip(), (task.get("js") or "").strip()]))
    ]
    if not code_tasks:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for task in code_tasks:
            ...
            if task_type == "frontend" and any([task.get("html"), task.get("css"), task.get("js")]):
                # Push separately for multi-pane
                all_ok = True
                last_error = ""
                if task.get("html"):
                    ok, err = await _upsert_github_file(client, owner, repo_name, branch, f"{base_dir}/frontend/{task_slug}/index.html", 
                                           base64.b64encode(task["html"].encode("utf-8")).decode("ascii"), f"Push html: {task_slug}", token)
                    if not ok: all_ok, last_error = False, err
                if task.get("css") and all_ok:
                    ok, err = await _upsert_github_file(client, owner, repo_name, branch, f"{base_dir}/frontend/{task_slug}/style.css", 
                                           base64.b64encode(task["css"].encode("utf-8")).decode("ascii"), f"Push css: {task_slug}", token)
                    if not ok: all_ok, last_error = False, err
                if task.get("js") and all_ok:
                    ok, err = await _upsert_github_file(client, owner, repo_name, branch, f"{base_dir}/frontend/{task_slug}/script.js", 
                                           base64.b64encode(task["js"].encode("utf-8")).decode("ascii"), f"Push js: {task_slug}", token)
                    if not ok: all_ok, last_error = False, err
                
                if not all_ok:
                    return None, None, pushed_count, last_error or f"Unable to push frontend files for {task_slug}"
                pushed_count += 1
            else:
                file_ext = "py" if task_type == "backend" else "html"
                code_dir = "backend" if task_type == "backend" else "frontend"
...
async def save_task(request: Request, task_id: str):
    ...
    try:
        ...
        code = html = css = js = None
    
    if any(x is not None for x in [code, html, css, js]):
        update_fields = {}
        if code is not None: update_fields["code"] = code
        if html is not None: update_fields["html"] = html
        if css is not None: update_fields["css"] = css
        if js is not None: update_fields["js"] = js
        ...
        await save_version(task_id, code, username, html=html, css=css, js=js)
...
async def save_task_history(request: Request, task_id: str):
    ...
    if not any([code, html, css, js]):
        print(f"[DEBUG] All fields are empty")
        raise HTTPException(status_code=400, detail="At least one code field must be provided")
    
    print(f"[DEBUG] Saving code to history. task_id: {task_id}, username: {username}")
    
    # Save the code version to history
    await save_version(task_id, code if code else "", username, html=html, css=css, js=js)
