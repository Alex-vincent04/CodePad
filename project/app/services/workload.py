...
async def enrich_developers_with_workload(db, developers: Iterable[dict]) -> list[dict]:
    ...
    # Collect IDs of completed projects so their tasks are excluded from workload
    completed_projects = await db.projects.find(
        {"status": "completed"}, {"_id": 1}
    ).to_list(length=5000)
    completed_project_ids = {str(p["_id"]) for p in completed_projects}

    tasks = await db.tasks.find({"assigned_to": {"$in": usernames}}).to_list(length=5000)
    # Exclude tasks that belong to a completed project
    tasks = [t for t in tasks if t.get("project_id") not in completed_project_ids]
